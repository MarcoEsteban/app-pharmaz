...
import { createUpdatePresent } from '@/actions';
...
interface Props {
  present: Partial<Presentacion>;
}

export const FormPresentacion = ( { present }: Props ) => {
  ...
  const onSubmit = async ( data: FormInputs ) => {
    ...
    if ( present.id ) {
      formData.append( 'id', present.id ?? '' );
    }
    formData.append( 'nombre', tipoToSave.nombre );

    const { ok, message } = await createUpdatePresent( formData );
