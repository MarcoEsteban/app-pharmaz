...
import { FaArrowLeft, FaFloppyDisk, FaPenToSquare, FaPlus, FaUnlockKeyhole, FaXmark, FaNotesMedical } from 'react-icons/fa6';
...
export const BtnModificar = ({ id }: { id: string }) => {
  ...
  return (
    <Link href={`${pathname}?modal=editar/${id}${pageParam}`} className="btn-gnrl gradient-update">
...
export const BtnVer = ({ id }: { id: string }) => {
  ...
  return (
    <Link href={`${pathname}?modal=ver/${id}${pageParam}`} className="btn-gnrl gradient-show">
      <FaEye size={18} />
    </Link>
  );
};
...
export const BtnPhoto = ({ id }: { id: string }) => {
  ...
  return (
    <Link href={`${pathname}?modal=foto/${id}${pageParam}`} className="btn-gnrl gradient-photo">
      <FaCamera size={18} />
    </Link>
  );
};
...
export const BtnPassword = ({ id }: { id: string }) => {
  ...
  return (
    <Link href={`${pathname}?modal=password/${id}${pageParam}`} className="btn-gnrl gradient">
...
export const BtnLote = ({ id }: { id: string }) => {
  ...
  return (
    <Link href={`${pathname}?modal=lote/${id}${pageParam}`} className="btn-gnrl gradient">
