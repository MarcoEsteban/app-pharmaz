...
export const getPaginationLotes = async ({
  ...
  try {
        ...
        query
          ? {
            OR: [
              ...
              { Producto: { laboratorios: { some: { Laboratorio: { nombre: { contains: query, mode: 'insensitive' }}}}}},
              { Producto: { presentacion: { some: { Presentacion: { nombre: { contains: query, mode: 'insensitive' }}}}}},
              { Producto: { principioActivo: { some: { PrincipioActivo: { nombre: { contains: query, mode: 'insensitive' }}}}}},
              { Producto: { viaAdministracion: { some: { ViaAdministracion: { nombre: { contains: query, mode: 'insensitive' }}}}}},
            ]
          } : {},
        categoria
          ? {
    ...
    const lotesData = lotes.map(lote => {
      ...
      const vencimiento = lote.vencimiento ? DateTime.fromJSDate(lote.vencimiento) : null;
      ...
      return {
        ...
        principioActivo: lote.Producto?.principioActivo?.[0]?.PrincipioActivo?.nombre || '',
