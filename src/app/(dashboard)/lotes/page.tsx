...
export default async function LotesPage({ searchParams }: Props) {
  ...
  const { lotes, totalPages } = await getPaginationLotes({
    currentPage,
    query,
  ...
  return (
    <>
      <Card>
        ...
        >
          <div
            className={"grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-4 pt-4 mb-2"}
          >
            {
              // ( [ ...Array( 8 ) ].map( ( _, index ) => (
              lotes.map((item) => <ItemCardLote lote={item} key={item.id} />)
            }
          </div>
        </Suspense>
